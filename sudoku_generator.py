...
class SudokuGenerator:
    ...
    def is_valid(self, row, col, num):
        if self.valid_in_row(row, num) and self.valid_in_col(int(col), num) and \
                self.valid_in_box(row//3 * 3, int(col)//3 * 3, num):
            return True
        return False
        ...
        while len(li) < 9:
            ...
            while temp < 3:
                ...
                if num in li:
                    ...
                    self.board[row_start][col_start + temp] = num
                    li.append(num)
                    temp += 1
            row_start += 1
